...
# --- Page Configuration ---
st.set_page_config(
    page_title="MycoVarP Flexible Pipeline",
    layout="wide",
...
st.markdown('<h1 class="main-title">MycoVarP Flexible Pipeline Dashboard</h1>', unsafe_allow_html=True)
...
current_config_display = {} # Initialize dict to store the config to display

with config_col1:
    if 'config_mode' not in st.session_state: st.session_state.config_mode = "Standard (Recommended)"
    mode = st.radio( "Configuration Mode:", ("Standard (Recommended)", "Advanced (Custom)"), key="config_mode", help="Choose 'Standard' for the validated MycoVarP workflow or 'Advanced' to customize." )
...
with config_col2:
    if st.session_state.config_mode == "Standard (Recommended)":
        ...
        cols = [adv_col1, adv_col2, adv_col3]; col_idx = 0
        temp_advanced_config = {}
        for key in OPTIONS.keys(): # Iterate through defined OPTIONS
             opts = OPTIONS[key]
             # Ensure key exists in STANDARD_CONFIG for default lookup, handle gracefully if not
             standard_val = STANDARD_CONFIG.get(key)

             with cols[col_idx % len(cols)]:
                default_index = 0
                if standard_val and standard_val in opts:
                    try: default_index = opts.index(standard_val)
                    except ValueError: pass
                elif opts: default_index = 0

                session_key = f"advanced_{key.lower().replace('/', '').replace(' ', '_').replace('.', '').replace('(','').replace(')','')}"
                if session_key not in st.session_state:
                     # Initialize state only if options exist
                     st.session_state[session_key] = opts[default_index] if opts else standard_val # Use standard_val if no options

                if opts: # Only create selectbox if there are options defined
                    selected_option = st.selectbox(
                        f"{key}:", opts, index=opts.index(st.session_state[session_key]),
                        key=f"sb_{session_key}", help=f"Choose the desired option for {key}."
                    )
                    st.session_state[session_key] = selected_option
                    temp_advanced_config[key] = selected_option
                else:
                    # If no options defined for this key in OPTIONS, just use standard
                    temp_advanced_config[key] = standard_val
             col_idx += 1
        current_config_display = temp_advanced_config
        display_current_config_compact(current_config_display)

st.markdown('</div>', unsafe_allow_html=True) # End styled div for config section
